...
class zcBootstrapBasic extends base
{
            ...
            foreach ($sort_array as $sort_key => $sort_val) {
                if (isset($sort_val['field'])) {
                    ...
                    if ($sniffer->field_exists($database_table, $sort_field)) {
                        $order_by_order = 'ASC';
                        if (isset($sort_val['order']) && strtoupper($sort_val['order']) == 'DESC') $order_by_order = 'DESC';
    ...
    function formDetails($tab, $id = 0)
    {
        ...
        while (!$columns->EOF) {
            ...
            if (!isset($field_array['display'])) $field_array['display'] = ucfirst(str_replace('_', ' ', $field_name));
    ...
    function buildFormHtml($form_array, $action_type = '')
    {
        ...
        foreach ($field_arrays as $field_array) {
            ...
            if (isset($max_length) && $max_length != 0) $parameters .= ' maxlength="' . $max_length . '"';
            $req_html = '';
            if ($required == true) {
                $parameters .= " required";
    ...
    public function getListingLayout($tab)
    {
        ...
        switch ($tab) {
            case 'defines':
                ...
                break;
            case 'options':
                $layout['order'] = array('bootstrap_basic_options_id', 'options_group', 'options_name', 'options_value');
