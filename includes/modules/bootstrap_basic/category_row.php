...
$get_started_button = '<br/><button type="button" class="btn buttonGetStarted">Get Started</button>';
$title = '';
...
if ($num_categories > 0) {
  ...
  while (!$categories->EOF) {
    ...
    $list_box_contents[$row][$col] = array('params' => 'class="col-xs-12 col-sm-6 col-md-4 col-lg-4 categoryListBoxContents"' ,
                                           'text' => '<a href="' . zen_href_link(FILENAME_DEFAULT, $cPath_new) . '">' . zen_image(DIR_WS_IMAGES . $categories->fields['categories_image'], $categories->fields['categories_name']) . '<br />' . $categories->fields['categories_name'] .$get_started_button. '</a>');
