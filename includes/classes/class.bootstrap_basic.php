 ...
 * @version Author: bislewl  9/11/2017 2:10 PM Modified in
 */
...
class bootstrap_basic extends base{
	...
	function __construct(){
		...
		$bootstrap_basic_options = $db->Execute("SELECT * FROM " . TABLE_BOOTSTRAP_BASIC_OPTIONS);
		if($bootstrap_basic_options->RecordCount() > 0){
			while(!$bootstrap_basic_options->EOF){
				define($bootstrap_basic_options->fields['options_define'], $bootstrap_basic_options->fields['options_value']);
				$bootstrap_basic_vars[$bootstrap_basic_options->fields['options_define']] = $bootstrap_basic_options->fields['options_value'];
				$bootstrap_basic_options->MoveNext();
			}
		}
...
//                }
		...
		if($item_type == 'cat_dropdown'){
			$html          = $this->getCategoryDropdown($category_id);
...
//			$dropdown .= '<span class="mobileDropDownClosed mobileDropDownChevron"><i class="fa fa-chevron-right"></i></span>';
	...
	public function categoryDropdownMenuMultiColumn($cat_id)
	{
		global $db;
		$main_cPath = zen_get_generated_category_path_rev((int)$cat_id);
		$cat_lookup = $db->Execute("SELECT c.categories_id, cd.categories_name, c.categories_image, c.parent_id FROM " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd 
        ...
        WHERE c.categories_id = cd.categories_id  AND c.categories_status=1  AND cd.language_id = '" . (int)$_SESSION['languages_id'] . "' AND parent_id='" . (int)$cat_id . "'" .
		                    " ORDER BY c.parent_id, c.sort_order, cd.categories_name";
		$categories = $db->Execute($categories_query);
		$dropdown_id = "dropdownCat-" . $main_cPath;
		$dropdown = '<li class="cat-dropdown dropdown">';
		$dropdown .= '<a href="' . zen_href_link(FILENAME_DEFAULT, 'cPath=' . $main_cPath) . '" id="' . $dropdown_id . '" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">';
		$dropdown .= $cat_lookup->fields['categories_name'];
		$dropdown .= '</a>';
		$dropdown .= '<div class="dropdown-menu multi-level" role="menu" aria-labelledby="' . $dropdown_id . '">';
		$dropdown .= '<div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">';
		$dropdown .= '<ul class="cat-dropdown-ul">';
		$cat_img = '';
		if ($categories->RecordCount() > 0) {
			while (!$categories->EOF) {
				$ci_cpath = $main_cPath . '_' . $categories->fields['categories_id'];
				$cat_li = '<li id="catDropdownLink-' . $categories->fields['categories_id'] . '">';
				$cat_li .= '<a href="' . zen_href_link(FILENAME_DEFAULT, 'cPath=' . $ci_cpath) . '" data-category-image="' . $categories->fields['categories_image'] . '">';
				$cat_li .= $categories->fields['categories_name'];
				$cat_li .= '</a>';
				$cat_li .= '</li>';
				$dropdown .= $cat_li;
				$cat_img .= zen_image(DIR_WS_IMAGES.$categories->fields['categories_image'], $categories->fields['categories_name'], SUBCATEGORY_IMAGE_WIDTH, SUBCATEGORY_IMAGE_HEIGHT, ' class="headerDropDownImage" id="catDropdownImage-' . $categories->fields['categories_id'] . '"');
				$categories->MoveNext();
			}
		}
		$dropdown .= '</ul>';
		$dropdown .= '</div>';
		$dropdown .= '<div class="hidden-xs hidden-sm col-md-6 col-lg-6">';
		$dropdown .= $cat_img;
		$dropdown .= '</div>';
		$dropdown .= '</div>';
		$dropdown .= '</li>';
