...
<?php if($COWOA) { ?>
  <div id="order_steps">
    <div class="order_steps_text">
      <span class="order_steps_text1_COWOA"><?php echo TEXT_ORDER_STEPS_BILLING; ?></span><span class="order_steps_text2_COWOA"><?php echo TEXT_ORDER_STEPS_1; ?></span><span id="active_step_text_COWOA"><?php echo ORDER_STEPS_IMAGE; ?><br /><?php echo TEXT_ORDER_STEPS_2; ?></span><span class="order_steps_text3_COWOA"><?php echo TEXT_ORDER_STEPS_3; ?></span><span class="order_steps_text4_COWOA"><?php echo TEXT_ORDER_STEPS_4; ?></span>
    </div>
...
<?php } else { ?>
  <div id="order_steps">
    <div class="order_steps_text">
      <span class="order_steps_text2"><?php echo TEXT_ORDER_STEPS_1; ?></span><span id="active_step_text"><?php echo ORDER_STEPS_IMAGE; ?><br /><?php echo TEXT_ORDER_STEPS_2; ?></span><span class="order_steps_text3"><?php echo TEXT_ORDER_STEPS_3; ?></span><span class="order_steps_text4"><?php echo TEXT_ORDER_STEPS_4; ?></span>
    </div>
    <div class="order_steps_line_2">
...
<!-- eof Order Steps (tableless) -->
    ...
    if (!$payment_modules->in_special_checkout()) {
        ...
        ?>
<legend class="use"><?php echo TITLE_BILLING_ADDRESS; ?></legend>
    ...
    if (sizeof($selection) > 0) {
        for ($i = 0, $n = sizeof($selection); $i < $n; $i++) {
            ...
            for ($j = 0, $n2 = sizeof($selection[$i]['fields']); $j < $n2; $j++) {
                ?>
            <?php if (!($selection[$i]['module'] == MODULE_ORDER_TOTAL_GV_TITLE)) { ?>
                    <fieldset>
<legend class="use"><?php echo $selection[$i]['module']; ?></legend>
...
<legend class="use"><?php echo TABLE_HEADING_PAYMENT_METHOD; ?></legend>
...
if (DISPLAY_CONDITIONS_ON_CHECKOUT == 'true') {
    ?>
        <fieldset>
<legend class="use"><strong><?php echo TABLE_HEADING_CONDITIONS; ?></strong></legend>
            <div style="font-size:14px;background:#FFFF00;border:1px solid #ccc;padding:10px"><?php echo TEXT_CONDITIONS_DESCRIPTION; ?>
            
            <br /><?php echo zen_draw_checkbox_field('conditions', '1', false, 'id="conditions"'); ?><label class="checkboxLabel" for="conditions"><?php echo TEXT_CONDITIONS_CONFIRM; ?></label></div><br /><i>This does not apply to special orders whose terms are laid out in advance.</i>
        </fieldset>
    <?php
}
?>
    ...
    <div class="buttonRow forward"><button type="submit" class="btn btn-success btn-lg" <?php echo 'onclick="submitFunction(' . zen_user_has_gv_account($_SESSION['customer_id']) . ',' . $order->info['total'] . ')"'; ?>>Send Order</button></div>
    <div class="buttonRow back"><button type="button" class="btn btn-default btn-lg">Back</button><br />' . TEXT_CONTINUE_CHECKOUT_PROCEDURE; ?></div>

</form><br class="clearBoth" /><br class="clearBoth" /><br class="clearBoth" />
</div>
